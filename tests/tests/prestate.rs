...
async fn test_initialization(state: DevRpcInitState) -> Result<()> {
    ...
    let balance = state.client.get_balance(state.accts[0].address(), None).await?;
...
pub static __ST1: Test<DevRpcInitState> = Test { name: "test_initialization", run: |s| Box::pin(test_initialization(s)) };
